      ...
      >
        ...
        <div className="flex items-center gap-2 sm:gap-3 text-xs text-gray-500 shrink-0">
          <span className="flex items-center gap-1">
            ...
            <span className="hidden xs:inline">{user.stamp_count} 印</span>
            <span className="xs:hidden">{user.stamp_count}</span>
          </span>
          <span className="flex items-center gap-1">
            <span className="w-2 h-2 rounded-full bg-purple-400 inline-block" />
            <span className="hidden xs:inline">{user.draw_count} 抽</span>
            <span className="xs:hidden">{user.draw_count}</span>
          </span>
          <span className="flex items-center gap-1">
                ...
                {configs.filter(c => ["02","05","06"].includes(c.stamp_id)).map(c => (
                  <div key={c.uuid} className="flex items-center justify-between p-3.5 border border-gray-100 rounded-xl hover:bg-orange-50 transition-colors">
                    <div className="flex items-center gap-3 min-w-0">
                      <div className="w-9 h-9 bg-orange-100 text-orange-700 rounded-lg flex items-center justify-center font-black text-sm shrink-0">
                        {c.stamp_id}
                      </div>
