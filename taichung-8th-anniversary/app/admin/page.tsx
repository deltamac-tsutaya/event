...
import {
  ...
  KeyRound, Eye, EyeOff, SlidersHorizontal, Shield,
} from "lucide-react";
    ...
    <div className="space-y-4 max-w-sm">
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-bold text-sm text-[#1A2B4A]">進階管理授權碼</h3>
          <span className="text-[9px] bg-[#C9A84C]/10 text-[#C9A84C] font-bold px-2 py-0.5 rounded-full border border-[#C9A84C]/20 ml-auto">獎項機率頁面</span>
        </div>
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-2xl px-4 py-3 text-[11px] text-amber-700 space-y-1">
        ...
        <p>· 此授權碼專用於「獎項機率管理」頁面的二次驗證</p>
        <p>· 與核銷 PIN 獨立，建議設定不同的授權碼</p>
        <p>· 通過驗證後，同一瀏覽器會話中有效；關閉分頁後需重新驗證</p>
      </div>
    </div>
  );
}
      ...
      <main className="mx-auto max-w-5xl px-4 py-5 sm:py-8 space-y-5">
        ...
        {activeTab === "settings" && (
          <div className="space-y-8">
            ...
            <div>
              <h2 className="text-sm font-bold text-gray-700 mb-4">進階授權碼</h2>
              <SuperPinSettings />
            </div>
          </div>
        )}
