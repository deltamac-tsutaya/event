...
export const metadata: Metadata = {
  title: "台中市政店優惠",
  description: "TSUTAYA BOOKSTORE 台中市政店母親節限定：MAGBLOX 9 折、沃堤思世界茶莊園 3 件 NT$990 等。",
  openGraph: {
    title: "台中市政店優惠 ・ 母親節特輯",
    description: "市政限定優惠＋雙店同步活動。",
  },
};

export default function TaichungPage() {
  const only = promotions.filter((p) => p.store === "taichung");
  ...
  return (
    <Layout>
        ...
        <div className="relative aspect-[16/9] md:aspect-[16/7] overflow-hidden">
          ...
          <div className="absolute bottom-0 left-0 right-0">
            <div className="mx-auto max-w-[1280px] px-5 md:px-10 pb-8 md:pb-14 text-paper">
              <p className="font-display italic text-sm tracking-[0.35em]">Store No. 02</p>
              ...
              <p className="mt-4 max-w-xl text-sm md:text-base leading-loose opacity-90">
                TSUTAYA BOOKSTORE 台中市政店 ・ 市政限定品牌與雙店同步優惠匯整。
              </p>
            </div>
          </div>
        </div>
      ...
      <section className="mx-auto max-w-[1280px] px-5 md:px-10 py-20 md:py-28">
        <SectionHead
          index="01"
          en="Taichung Exclusive"
          zh="市 政 限 定"
          description="僅限 TSUTAYA BOOKSTORE 台中市政店之品牌優惠。"
        />
        <div className="mt-10">
          {only.map((p, i) => (
      ...
      <section className="border-t border-rule bg-paper-deep/30">
        <div className="mx-auto max-w-[1280px] px-5 md:px-10 py-20 md:py-28">
          <SectionHead
            index="02"
            en="Also Available"
            zh="雙 店 同 步 ・ 市 政 亦 適 用"
            description="以下品牌優惠在台中市政店亦可享有。"
          />
          <div className="mt-10">
            {both.map((p, i) => (
              <PromoCard key={p.id} promo={p} index={i + 1} />
